...
The recording-style `ScriptBuilder.assert_*` API has been replaced with a real-execution API plus an idiomatic Vitest `expect.extend` matcher set. The custom `BatsRunner` is removed.
...
- Recorder shim heredoc body emitted without indentation so the generated shim's shebang sits at column 0
